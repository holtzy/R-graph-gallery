...
The region boundaries required to make maps are usually stored in geospatial objects. Those objects can come from [shapefiles](168-load-a-shape-file-into-r.html), [geojson files](325-background-map-from-geojson-format-in-r.html) or provided in a [R package](278-the-maps-library.html). See the [background map](maps.html) section for possibilities.
...
library(sf)
world_sf <- read_sf(paste0(getwd(),"/DATA/world_shape_file/","TM_WORLD_BORDERS_SIMPL-0.3.shp"))

# Clean the data object
library(dplyr)
world_sf <- world_sf %>%
  mutate(POP2005 = ifelse(POP2005 == 0, NA, 
                          round(POP2005/ 1000000, 2))
  )
...
mypalette <- colorNumeric( palette="viridis", domain=world_sf$POP2005, na.color="transparent")
mypalette(c(45,43))
...
# Distribution of the population per country?
  ggplot(world_sf, aes(x=as.numeric(POP2005))) + 
...
```{r, eval=FALSE}
# Color by quantile
m <- leaflet(world_sf)%>% addTiles()  %>% setView( lat=10, lng=0 , zoom=2) %>%
...
mypalette <- colorBin( palette="YlOrBr", domain=world_sf$POP2005, na.color="transparent", bins=mybins)
