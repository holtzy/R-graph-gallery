...
library(sf)
my_sf <- st_read(tmp_geojson,  quiet = TRUE)
```
...
It is totally possible (and advised IMO) to build the map with [ggplot2](ggplot2-package.html), using the `geom_polygon()` function as described below.
