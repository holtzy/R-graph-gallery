...
This step has been extensively describe in [chart #325](325-background-map-from-geojson-format-in-r/). The sf library allows to read this type of format in R. To plot it with ggplot2, the geom_sf() function allows to represent this type of object !
...
my_sf <- my_sf[ substr(my_sf$code,1,2)  %in% c("06", "83", "13", "30", "34", "11", "66") , ]
```
...
On `ggplot2` we can use `geom_sf` to plot the shape.
...
my_sf_merged$nb_equip[is.na(my_sf_merged$nb_equip)] <- 0.01
...
p <- ggplot(my_sf_merged) +
  ...
  theme(
    ...
    plot.caption = element_text( size=10, color = "#4e4d47", margin = margin(b = 0.3, r=-99, t=0.3, unit = "cm") ),
