...
When you have lots of data points, it can be useful to change the **position behavior** of the dots. 

Let's start by adding more data points to the `iris` dataset to illustrate this:


```{r, echo=TRUE}
# add 6 times more data points
iris_large <- do.call(rbind, replicate(6, iris, simplify = FALSE))
```

Let's now create a beeswarm plot with the default position behavior:
...
As you can see, the dots are now overlapping. Fortunately, the `corral` argument allows you to change this. The available options are:
...
You might also be interested in [how to create a beeswarm plot with ggplot2](373-beeswarm-plot-with-ggplot2.html).
