...
descriptionTop: "This post explains how to use the [ggiraph](package/ggiraph.html) package to create **multiple interactive graphs**, from very simple example to more complex ones.<br><br>For an introduction to `ggiraph`, check the [dedicated post](package/ggiraph.html)"
...
In order to **map the hover effect** between multiple charts, we have to use the same value for the `data_id` argument. In our case, we want our mapping at the **car level**, using the `mtcars` dataset. 
...
With the `data_id` argument, we can craft **mini dashboards** featuring numerous charts.

As before, we generate **three distinct charts** using the same `data_id` (in this case, the country) to form a cohesive figure with three **interconnected charts**  !
...
In this example, we'll use the `girafe_options()` function to provide **additionnal CSS** when hovering the charts. 
...
# Create the third chart (Lollipop)
...
interactive_plot <- interactive_plot %>%
  girafe_options(
    ...
    opts_tooltip(css = "border: 2px solid #4CAF50; border-radius: 10px; box-shadow: 2px 2px 12px rgba(0, 0, 0, 0.2); background-color: #f9f9f9; padding: 10px;")
  )
