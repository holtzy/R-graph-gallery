...
library(sf)
my_sf  <- read_sf(paste0(getwd(), "/DATA/world_shape_file/TM_WORLD_BORDERS_SIMPL-0.3.shp"))
...
# Plot
plot(africa$geometry , xlim=c(-20,60) , ylim=c(-40,40))
```
...
In this example, this variable is already available in the `my_sf` object. You can have a look to it typing `my_sf` in the terminal.
...
# Make the plot
plot(africa$geometry , xlim=c(-20,60) , ylim=c(-40,40), col=my_colors ,  bg = "#A6CAE0")
```
