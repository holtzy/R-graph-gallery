...
descriptionTop: "A blogpost showing how to use custom fonts in `R` and `ggplot2`. Two different approaches are shown. The first makes use of the `showtext` package, and the second uses the AGG backend provided by `ragg`."
...
quality of a chart. [`showtext`](https://github.com/yixuan/showtext) and 
[`ragg`](https://ragg.r-lib.org/) are two R packages that help to work with custom
fonts in R and `ggplot2`. `showtext` provides a set of functions that make it easy
to use various types of fonts in R plots, and `ragg` provides graphic devices 
...
makes `showtext` work). See how simple it is in practice:
...
If the font wanted is not available on Google Fonts, one can use `font_add()`.
The first argument is like the `family` above, and the second argument
is a path to the font file for the regular font face (both `.ttf` and `.otf` work)
...
`showtext` is going to be automatically invoked to draw text whenever a plot
is created. 
...
However, it's good to note some caveats with `showtext` before jumping to the 
next section:

* If you are using `showtext` in a common R script, set `dpi` according to the 
...
* If you are using `showtext` in RMarkdown documents you don't have to
