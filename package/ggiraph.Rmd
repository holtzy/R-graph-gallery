...
Here's an example of the `geom_point_interactive()` function that "replaces" the original `geom_point()` from [ggplot2](../ggplot2-package.html). Then we specify `hover_nearest = TRUE` to always consider the **nearest point** to be hovered.
...
myplot <- ggplot(
  ...
  mapping = aes(
    x = disp, y = qsec,
    # here we add ibteractive aesthetics
...
p_hdi_atlas <- pmap + inset_element(pcol, left = 0.62, bottom = 0.10, right = 0.99, top = 0.90)
p_hdi_atlas <- pmap + pcol
...
<iframe src="../HtmlWidget/ggiraph-6.html" height="800" width="750" style="border:none;"></iframe>
