...
logo: "sf"
descriptionMeta: "The sf package in R specializes in geospatial visualization, offering streamlined access to geographic features like points, lines, and polygons."
descriptionTop: "The `sf` package in R specializes in **geospatial visualization**, offering streamlined access to **geographic features** like points, lines, and polygons."
...
In just a few lines of code, you can create stunning visualizations with the `sf` package in R. You only need to [load a **shapefile**](../168-load-a-shape-file-into-r.html) and you're ready to go!
...
The `sf` package allows you to create [choropleth maps](../choropleth-map.html) with ease. Simply use the `plot()` function to visualize your data.
...
plot(ncg, border = 'grey', main = 'Centroids of North Carolina')
plot(cntrd, col = 'red', add = TRUE, cex = 0.6, pch = 8)
```
