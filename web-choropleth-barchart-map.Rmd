...
descriptionTop: "This post describes how to build a choropleth map together with a bar chart in R using `ggplot2` and `patchwork`."
...
As a teaser, here is the map we'll learn how to build. This visualization shows the Human Development Index (HDI) at the subregional level in Sao Paulo, Brazil's largest city. The values follow the standard United Nations's HDI: values are in the 0 to 1 range. The visualization combines a choropleth map with a bar chart using the `patchwork` package.
