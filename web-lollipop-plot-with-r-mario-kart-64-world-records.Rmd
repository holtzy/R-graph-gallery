...
We start by loading the packages needed to build the figure. From all the 
packages we use, [`ragg`](https://ragg.r-lib.org/index.html) may be the least 
known. It is worth noting it a great library that impacts both the quality and 
...
shortcuts from those records with shortcuts. The same logic is used in the next 
