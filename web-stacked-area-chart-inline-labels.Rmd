...
You are probably familiar with the `tidyverse` already. `readxl` will be used in the next section to load the dataset from a `xlsx` format directly. `ggstream` is used to smooth the area shapes. [showtext](package/showtext.html) is used to load some custom fonts.
...
This is made possible thanks to the [showtext](package/showtext.html) package and its `font_add_google()` function. Using custom fonts can be a bit tricky. Fortunately, I wrote a [complete tutorial](https://r-graph-gallery.com/custom-fonts-in-R-and-ggplot2.html) just in case the following code sounds strange to you.
...
caption_text <- str_glue(
  "**Data:**  James Davies, Rodrigo Lluberas and Anthony Shorrocks, Credit Suisse Global Wealth Databook 2022<br>",
...
plot <- df %>%
  ...
  ylab("") +
  labs(
    caption = caption_text # "Data: Flash Eurobarometer, Number 509 (October 2022)"
  ) +
  theme(
